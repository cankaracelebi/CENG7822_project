...
Quick test:
    python shooter_env.py

Optional SB3 training (if you want):
...
# Pygame is only required for rendering + simulation loop
import pygame
...
class ShooterEnv(gym.Env):
    ...
    def _compute_reward(self) -> float:
        # Base shaping values (tuned to be stable)
        R_PRIZE = 1.0
        R_HIT = 0.3
        R_KILL = 1.0
    ...
    def render(self):
        ...
        if self._screen is None:
            ...
            if self.render_mode == "human":
                ...
                pygame.display.set_caption("ShooterEnv")
            else:
        ...
        # Draw prizes
        for p in self.prizes:
            pygame.draw.circle(self._screen, PRIZE_C, (int(p.x), int(p.y)), int(p.radius))

        # Draw enemies
        for e in self.enemies:
            pygame.draw.circle(self._screen, ENEMY_C, (int(e.x), int(e.y)), int(e.radius))

        # Draw bullets
        for b in self.bullets:
            pygame.draw.circle(self._screen, BULLET_C, (int(b.x), int(b.y)), int(b.radius))

        # Draw agent
        pygame.draw.circle(self._screen, AGENT_C, (int(self.agent.x), int(self.agent.y)), int(self.agent.radius))

        # HUD (simple health bar)
        bar_w, bar_h = 180, 10
        x0, y0 = 12, 12
        pygame.draw.rect(self._screen, (60, 60, 60), pygame.Rect(x0, y0, bar_w, bar_h))
        fill = int(bar_w * clamp(self.agent.health, 0, 1))
        pygame.draw.rect(self._screen, AGENT_C, pygame.Rect(x0, y0, fill, bar_h))

        # Text
        if self.render_mode == "human":
            font = pygame.font.SysFont("Arial", 16)
            txt = f"HP: {self.agent.health:.2f}  Enemies: {len(self.enemies)}  Prizes: {len(self.prizes)}  Step: {self._step_count}"
            surf = font.render(txt, True, HUD_C)
            self._screen.blit(surf, (12, 28))
        ...
        # rgb_array
        array = pygame.surfarray.array3d(self._screen)  # (W,H,3)
        # Convert to (H,W,3)
...
def run_random_episode(render: bool = True):
    ...
    while not (terminated or truncated):
        ...
        # Let pygame process window events
        if render:
...
# ----------------------------
# Optional: SB3 training snippet
# ----------------------------

def train_with_sb3_demo(timesteps: int = 200_000):
    """
    ...
    def make_env():
        return ShooterEnv(render_mode=None)
    ...
    # Test visually
    test_env = ShooterEnv(render_mode="human")
    ...
    test_env.close()
    print("Trained episode return:", ep_ret)


if __name__ == "__main__":
    # Default: run a random episode with rendering.
    run_random_episode(render=True)

    # Uncomment to train (requires stable-baselines3).
    #train_with_sb3_demo(200_000)
